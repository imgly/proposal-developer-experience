...
export type CreativeEditorProps = Omit<
...
function CreativeEditor(props: CreativeEditorProps) {
  ...
  const initializeEditor = useCallback(
    props.onInitialize || defaultInitialize,
    [props.onInitialize],
  );
  ...
  useEffect(() => {
    ...
    CreativeEditorSDK.create(cesdk_container.current, config)
      .then(async (cesdk: CreativeEditorSDK) => {
        ...
        try {
          ...
          // Ensure a scene is created if initialization didn't create one
          if (!cesdk.engine.scene.get()) {
            await cesdk.createDesignScene();
          }
      ...
      .catch((err) => {
        ...
        setError(
          err instanceof Error ? err : new Error("Failed to create editor"),
        );
        setIsLoading(false);
      });
